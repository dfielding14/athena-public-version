...
EquationOfState::EquationOfState(MeshBlock *pmb, ParameterInput *pin) {
  ...
  neighbor_flooring_ = pin->GetOrAddBoolean("hydro","neighbor_flooring_", false); // average the neighboring cells to get floor value rather than reset to the floor value
}
    ...
    for (int i=il; i<=iu; ++i) {
      ...
      if (neighbor_flooring_ == false) {
        ...
        if (w_p < pressure_floor_) {
          ...
          u_e = (p_neighbors/n_neighbors > pressure_floor_) ?  u_e : ((pressure_floor_/gm1) + ke + pb);
