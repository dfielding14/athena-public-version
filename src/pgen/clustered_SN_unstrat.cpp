...
// turb_flag = 2 for driven turbulence
  ...
  v_max                   = pin->GetOrAddReal("problem", "v_max", 1.0e4);  
  ...
  for (int k = ks; k <= ke; ++k) {
    ...
    for (int j = js; j <= je; ++j) {
      ...
      for (int i = is; i <= ie; ++i) {
        ...
        //   std::cout << " v1 > v_max, v1 = " << v1 << " x y z = " << x << " " << y << " " << z << " " <<
        ...
        //     "\n";
        //   if ((k>1)and(k<ke-1)and(j>1)and(j<je-1)and(i>1)and(i<ie-1)){
        //     rho = (cons(IDN,k+1,j,i) + cons(IDN,k-1,j,i) +
        //            cons(IDN,k,j+1,i) + cons(IDN,k,j-1,i) +
        //            cons(IDN,k,j,i+1) + cons(IDN,k,j,i-1))/6.;
        //     e   = (cons(IEN,k+1,j,i) + cons(IEN,k-1,j,i) +
        //            cons(IEN,k,j+1,i) + cons(IEN,k,j-1,i) +
        //            cons(IEN,k,j,i+1) + cons(IEN,k,j,i-1))/6.;
        //     m1  = (cons(IM1,k+1,j,i) + cons(IM1,k-1,j,i) +
        //            cons(IM1,k,j+1,i) + cons(IM1,k,j-1,i) +
        //            cons(IM1,k,j,i+1) + cons(IM1,k,j,i-1))/6.;
        //     m2  = (cons(IM2,k+1,j,i) + cons(IM2,k-1,j,i) +
        //            cons(IM2,k,j+1,i) + cons(IM2,k,j-1,i) +
        //            cons(IM2,k,j,i+1) + cons(IM2,k,j,i-1))/6.;
        //     m3  = (cons(IM3,k+1,j,i) + cons(IM3,k-1,j,i) +
        //            cons(IM3,k,j+1,i) + cons(IM3,k,j-1,i) +
        //            cons(IM3,k,j,i+1) + cons(IM3,k,j,i-1))/6.;
        //   }
        // }
