    ...
    for (int j = jl; j <= ju; ++j) {
      ...
      for (int i = il; i <= iu; ++i) {
        ...
        if (block_size.nx3 > 1) {
          ...
          phydro->w(IVY,k,j,i) = velocity * ( 1.0 - 0.5 * ( std::tanh((x-z_bot)/smoothing_thickness) - std::tanh((x-z_top)/smoothing_thickness) ) );
          phydro->w(IVZ,k,j,i) = 0.0;
        }
      }
    }
  ...
  for (int k=ks; k<=ke; ++k) {
    for (int j=js; j<=je; ++j) {
      for (int i=is; i<=ie; ++i) {
        ...
        grad_vel = sqrt(dvel2_dx1*dvel2_dx1 + dvel3_dx1*dvel3_dx1 
                      + dvel1_dx2*dvel1_dx2 + dvel3_dx2*dvel3_dx2 
  ...
  for (int k=ks; k<=ke; ++k) {
    for (int j=js; j<=je; ++j) {
      for (int i=is; i<=ie; ++i) {
        ...
        dvel2_dx1 = (prim(IVY,k,j,i) - prim(IVY,k,j,i-1))/pmb->pcoord->dx1v(i);
        dvel3_dx1 = (prim(IVZ,k,j,i) - prim(IVZ,k,j,i-1))/pmb->pcoord->dx1v(i);
        ...
        std::cout << " prim(IVX,k,j,i) = " << prim(IVX,k,j,i) << " prim(IVY,k,j,i) = " << prim(IVY,k,j,i) << " prim(IVZ,k,j,i) = " << prim(IVZ,k,j,i) << 
                     " pmb->pcoord->dx1v(i) = " << pmb->pcoord->dx1v(i) << 
                     " i = " << i << 
                     " grad_vel = " << grad_vel << 
                     "\n";
      }
